  ...
  actions: {
    init() {
      ...
      setInterval(() => {
        ...
        // 排除午休时间
        if (
          this.minutesPassed <= this.lunchStartMinutes ||
          this.minutesPassed >= this.lunchEndMinutes
        ) {
          // 判断当前是否已过午休
    ...
    updateParams() {
      ...
      if (Object.keys(setting).length !== 0) {
        ...
        this.salaryAfterTax = setting.salaryAfterTax
      }
      // 本月考勤日
      ...
      // 入职天数
      this.daysWorked = getWorkingDays(
        `${this.entryDateYear}-${this.entryDateMonth}-${this.entryDateDay}`
      )
    ...
    saveSetting() {
      const setting = {
        ...
        salaryAfterTax: this.salaryAfterTax
      }
